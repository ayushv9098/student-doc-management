...
export default function Home() {
  ...
  // ✅ Auth check – sabse pehle yeh chalega
  useEffect(() => {
  ...
  // ✅ Students load – tabhi jab auth confirm ho
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        setRecent(sorted.slice(0, 20).map((s) => ({
          id: s.id,
          fullName: s.fullName,
          mobile: s.mobile,
  ...
  // ⏳ Loading state – jab tak auth check ho raha
  if (!authChecked) {
    return (
      <div className="flex h-screen items-center justify-center">
        <div className="text-center">Checking login...</div>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-6xl px-3 py-6 sm:px-4 sm:py-8">
      <div className="flex flex-col gap-4 border-b border-zinc-200 pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-600 dark:text-zinc-300">Black/white minimal UI with modern layout.</p>
        </div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Card className="min-h-[200px] lg:col-span-1">
          ...
          <CardContent>
            {loading ? <div className="h-10 animate-pulse rounded bg-zinc-200 dark:bg-zinc-800" /> : <div className="text-4xl font-bold">{count}</div>}
          </CardContent>
        </Card>
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <p className="text-sm text-zinc-600 dark:text-zinc-300">Latest students (scroll to view more).</p>
          </CardHeader>
          <CardContent className="pt-3">
            {loading ? (
              ...
              <div className="rounded-md border border-zinc-200 bg-zinc-50 p-4 text-sm">No students yet. Click "Add" to create your first student.</div>
            ) : (
              <div className="max-h-[300px] space-y-2 overflow-y-auto">
                {recent.map((s) => (
                  <div key={s.id} className="flex items-center justify-between gap-3 rounded-md border border-zinc-200 bg-zinc-50 p-3 hover:bg-zinc-100">
                    <div>
